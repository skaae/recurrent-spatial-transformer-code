...
output_folder = "logs/FFN_SPN" + str(uuid.uuid4())[:18].replace('-', '_')
...
#mnist_sequence = "/Users/sorensonderby/Documents/phd/DATASETS/MNIST/mnist_sequence3_sample_8distortions_9x9.npz"
...
l_dense_loc = lasagne.layers.DenseLayer(l_conv2_loc, num_units=200, W=W_ini,
                                        name='l_dense_loc')
...
# note that the transformer layer is working on the input.
l_transform = lasagne.layers.TransformerLayer(
    incoming=l_dim,
    localization_network=l_A_net,
    downsample_factor=args.downsample)

l_conv0_out = conv(l_transform, num_filters=96, filter_size=(3, 3),
...
l_conv1_out = conv(l_drp1_out, num_filters=96, filter_size=(3, 3),
...
l_conv2_out = conv(l_drp2_out, num_filters=96, filter_size=(3, 3),
                   name='l_conv2_out', W=W_ini)

l_drp3_out = lasagne.layers.DropoutLayer(l_conv2_out, p=sh_drp)
l_conv3_out = conv(l_drp3_out, num_filters=96, filter_size=(3, 3),
                   name='l_conv3_out', W=W_ini)

l_class1 = lasagne.layers.DenseLayer(l_conv2_out, num_units=400,
                                     W=W_class_init,
                                     name='denseout')


l_lin_out1 = lasagne.layers.DenseLayer(l_class1, num_units=num_classes,
                                      W=W_class_init,
                                      name='class1', nonlinearity=lasagne.nonlinearities.softmax)
l_lin_out2 = lasagne.layers.DenseLayer(l_class1, num_units=num_classes,
                                      W=W_class_init,
                                      name='class2', nonlinearity=lasagne.nonlinearities.softmax)
l_lin_out3 = lasagne.layers.DenseLayer(l_class1, num_units=num_classes,
                                      W=W_class_init,
                                      name='class3', nonlinearity=lasagne.nonlinearities.softmax)


l_concat = lasagne.layers.ConcatLayer([l_lin_out1, l_lin_out2, l_lin_out3], axis=1)
l_out = lasagne.layers.ReshapeLayer(l_concat, (-1, 3, num_classes))


test = lasagne.layers.get_output(l_out, sym_x).eval({sym_x: x_train[:num_batch]})

output_train = lasagne.layers.get_output(
    l_out, sym_x, deterministic=False)
output_eval, l_A_eval = lasagne.layers.get_output([l_out, l_A_net], sym_x, deterministic=True)
...
if MONITOR:
    ...
    f_train = theano.function([sym_x, sym_y], [cost, output_train, norm],
                              updates=updates)
f_eval = theano.function([sym_x], [output_eval, l_A_eval.reshape((-1, 6))])
